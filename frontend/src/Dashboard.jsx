...
import { useSensorData, useDeviceAck, useMQTTPublish, useMQTTSubscribe } from "./mqtt-hooks";
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={styles.btnRow}>
        <button style={styles.btnOn}  onClick={() => publish(`${device}/${type}`, "ON")}>ON</button>
        <button style={styles.btnOff} onClick={() => publish(`${device}/${type}`, "OFF")}>OFF</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...styles.card, borderTop: `3px solid ${color}` }}>
      ...
      <span style={{ fontSize: 32, fontWeight: 700, color }}>
        {value ?? "–"}<span style={{ fontSize: 14, color: "#94a3b8" }}> {unit}</span>
      </span>
    </div>
  );
}
...
export default function Dashboard() {
  const sensor   = useSensorData(DEVICE);
  const [history, setHistory] = useState([]);
  const [alerts,  setAlerts]  = useState([]);
  const [ruleFire, setRuleFire] = useState([]);

  // ── Socket.IO for live history & anomaly events ──
  useEffect(() => {
    // Fetch last 60 historical readings from REST API
  ...
  // Live sensor → append to chart history
  ...
  // Anomaly notifications via Socket.IO (or simulate via REST polling)
  // Here we subscribe to a custom MQTT topic that backend can publish alerts to
  ...
  return (
    <div style={styles.page}>
      ...
      <p style={styles.sub}>Device: <b>{DEVICE}</b> · Live MQTT</p>

      {/* ── Sensor readings ── */}
      <section style={styles.row}>
        <SensorCard label="Temperature" value={sensor?.temp}     unit="°C"    color="#f97316" />
        <SensorCard label="Humidity"    value={sensor?.humidity} unit="%"     color="#3b82f6" />
        <SensorCard label="Motion"      value={sensor ? (sensor.motion ? "YES" : "NO") : null} unit="" color="#a855f7" />
      </section>
      ...
      {alerts.length > 0 && (
        <div style={styles.alertBox}>
          ...
          {alerts.map((a, i) => (
            <div key={i} style={styles.alertItem}>[{a.ts}] {a.msg}</div>
          ))}
        </div>
      )}
      ...
      {ruleFire.length > 0 && (
        <div style={styles.alertBox}>
          ...
          {ruleFire.map((r, i) => (
            <div key={i} style={styles.alertItem}>[{r.ts}] {r.msg}</div>
          ))}
        </div>
      )}
    </div>
...
// ─── INLINE STYLES ───────────────────────────────────────
const styles = {
  page:         { background: "#0f172a", minHeight: "100vh", padding: "24px 32px", fontFamily: "'JetBrains Mono', monospace", color: "#e2e8f0" },
  ...
  card:         { background: "#1e293b", borderRadius: 12, padding: "20px 24px", minWidth: 160, flex: 1, display: "flex", flexDirection: "column", gap: 10 },
  cardLabel:    { fontSize: 12, color: "#64748b", textTransform: "uppercase", letterSpacing: 1 },
  badge:        { display: "inline-block", borderRadius: 6, padding: "2px 10px", fontSize: 12, fontWeight: 700, alignSelf: "flex-start" },
  btnRow:       { display: "flex", gap: 8 },
  btnOn:        { flex: 1, padding: "8px 0", borderRadius: 8, border: "none", background: "#22c55e", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 13 },
  btnOff:       { flex: 1, padding: "8px 0", borderRadius: 8, border: "none", background: "#ef4444", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 13 },
  chartBox:     { background: "#1e293b", borderRadius: 12, padding: 24, marginBottom: 24 },
  sectionTitle: { fontSize: 14, fontWeight: 700, marginBottom: 16, textTransform: "uppercase", letterSpacing: 1, color: "#94a3b8" },
  alertBox:     { background: "#1e293b", borderRadius: 12, padding: 20, marginBottom: 20 },
  alertItem:    { fontSize: 13, padding: "6px 0", borderBottom: "1px solid #0f172a", color: "#fbbf24" },
};
