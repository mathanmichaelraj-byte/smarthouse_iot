...
function normalizeSensorPayload(message) {
  ...
  if (Number.isNaN(temp) || Number.isNaN(humidity) || motion === null) {
    return null;
  }
  ...
  return {
    ...
    humidity,
    motion,
    light1: normalizeBoolean(parsed.light1) ?? homeState.light1,
    light2: normalizeBoolean(parsed.light2) ?? homeState.light2,
    fan1: normalizeBoolean(parsed.fan1) ?? homeState.fan1,
    fan2: normalizeBoolean(parsed.fan2) ?? homeState.fan2,
    timestamp,
    receivedAt,
...
function anomalyMessage(snapshot, anomalyResult) {
  return `Anomaly detected: temp=${snapshot.temp}C humidity=${snapshot.humidity}% motion=${
    snapshot.motion ? "YES" : "NO"
...
async function handleSensorMessage(payload) {
  ...
  homeState.light1 = sensorSnapshot.light1;
  homeState.light2 = sensorSnapshot.light2;
  homeState.fan1 = sensorSnapshot.fan1;
  homeState.fan2 = sensorSnapshot.fan2;
  ...
  if (isNightHour(sensorSnapshot.hour) && sensorSnapshot.motion && !sensorSnapshot.light1 && !sensorSnapshot.light2) {
    await logAlert("Suspicious activity", "security", { buzzer: true });
  }
  ...
  const decision =
    normalizedPrediction && normalizedPrediction.confidence > 0.7
      ? normalizedPrediction
      : buildFallbackDecision(sensorSnapshot);

  await applyDecision(decision, normalizedPrediction && normalizedPrediction.confidence > 0.7 ? "ml" : "fallback");
}
